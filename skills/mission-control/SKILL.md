...
| `control-shell-hook.sh` | **the only thing that makes Control's `@` header right from birth.** Installs a shell function so a bare `claude` in a git repo launches as that repo's coordinator — the user types what they always typed. Fires ONLY on a bare `claude` at a terminal (the harness re-execs itself as `claude daemon run` / `bg-pty-host`, and injecting a flag into those would corrupt its plumbing); never claims a name a live session answers to; silent in a station worktree. `--install` / `--uninstall` / `--print` | the user, once per machine |
...
| `statusline.sh` | optional — wire it to `statusLine` in `~/.claude/settings.json` **with `refreshInterval: 5`** (everything it reports lives in another session, so local events never refresh it) to see this repo's live stations under the prompt (`fleet · CONTROL · BACKEND · CHANNELS · +1 unidentified`), in the footer's own grammar — **white = background, Claude orange = tab, golden yellow = its own window**, and **your own station is boxed** so you can see which tab you are in (measured from the registry, never from the spawn preference), **bold** for busy and dimmed for idle, coordinator leftmost then identification order. A user-level setting: one `statusLine` key in `~/.claude/settings.json`, not per-repo. **Counted, never named** if a session only has a generated handle, and **silent** for a lone unidentified session. **Spawns nothing**: reads the session registry and the sockets, never `claude agents` | anyone |
| `mc-config.sh` | `/mc-config` — shows every preference and changes any of them in place, and keeps `settings.json`'s `env.MC_SPAWN_MODE` in agreement so the two cannot disagree. **Delegates `spawn.mode` and `tour` to the scripts that own them** rather than writing those keys itself | Control |
