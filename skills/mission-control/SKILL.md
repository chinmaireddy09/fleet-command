...
| `statusline.sh` | optional — wire it to `statusLine` in `~/.claude/settings.json` to see this repo's live stations under the prompt (`fleet · BACKEND · CHANNELS (bg) · +1 unidentified`), in the footer's own grammar — bold is busy. **Counted, never named** if a session only has a generated handle, and **silent** for a lone unidentified session. **Spawns nothing**: reads the session registry and the sockets, never `claude agents` | anyone |
