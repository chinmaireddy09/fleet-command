...
| `statusline.sh` | optional — wire it to `statusLine` in `~/.claude/settings.json` **with `refreshInterval: 5`** (everything it reports lives in another session, so local events never refresh it) to see this repo's live stations under the prompt (`fleet · CONTROL · BACKEND · CHANNELS · +1 unidentified`), in the footer's own grammar — **gold = background, purple = tab, turquoise = its own window**, and **your own station is boxed** so you can see which tab you are in (measured from the registry, never from the spawn preference), **bold** for busy and dimmed for idle, coordinator leftmost then identification order. A user-level setting: one `statusLine` key in `~/.claude/settings.json`, not per-repo. **Counted, never named** if a session only has a generated handle, and **silent** for a lone unidentified session. **Spawns nothing**: reads the session registry and the sockets, never `claude agents` | anyone |
