...
| `statusline.sh` | optional — wire it to `statusLine` in `~/.claude/settings.json` to see this repo's live stations under the prompt (`fleet · BACKEND · CHANNELS (bg) · +1 unidentified`), in the footer's own grammar — **one stable colour per call-sign**, full brightness for busy and dimmed for idle. **Counted, never named** if a session only has a generated handle, and **silent** for a lone unidentified session. **Spawns nothing**: reads the session registry and the sockets, never `claude agents` | anyone |
