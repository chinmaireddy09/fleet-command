...
| `statusline.sh` | optional — wire it to `statusLine` in `~/.claude/settings.json` to see this repo's live stations by call-sign under the prompt. **Spawns nothing**: reads the session registry and the sockets, never `claude agents` | anyone |
