...
if [ ! -f "$SL" ]; then sk "status line not shipped in this copy"; else
  ...
  case "$(slrender)" in *$'\033[2;38;5;173mCONTROL'*) no "a busy station is never dimmed" "$(slrender | cat -v)" ;; *) ok "a busy station is never dimmed" ;; esac
  ...
  case "$(slrender)" in *$'\033[1;38;5;173mSHELLED'*) ok "a station running a shell is working" ;; *) no "a station running a shell is working" "$(slrender | cat -v)" ;; esac
  ...
  case "$(slrender)" in *$'\033[2;38;5;173mNOSTATUS'*) ok "a station with no status stays dim" ;; *) no "a station with no status stays dim" "$(slrender | cat -v)" ;; esac
  ...
  case "$(slrender)" in *$'\033[1;38;5;173mCONTROL'*|*$'\033[1;38;5;173mCONTROL'*) ok "a busy station is bold" ;; *) no "a busy station is bold" "$(slrender | cat -v)" ;; esac
  case "$(slrender)" in *$'\033[1;38;5;173mFRONTEND'*|*$'\033[1;38;5;231mFRONTEND'*) no "an idle one is never bold" "$(slrender | cat -v)" ;; *) ok "an idle one is never bold" ;; esac
  ...
  BAD=$(printf '%s' "$HUES" | tr ' ' '\n' | grep -v '^$' | grep -vE '^38;5;(173|220|231)$' || true)
  ...
  case "$(slrender)" in *$'\033[2;38;5;231mCHANNELS'*) ok "a background station is white" ;; *) no "a background station is white" "$(slrender | cat -v)" ;; esac
  case "$(slrender)" in *$'\033[2;38;5;173mFRONTEND'*) ok "a tab station is Claude orange" ;; *) no "a tab station is Claude orange" "$(slrender | cat -v)" ;; esac
  ...
  case "$(slrender)" in *$'\033[2;38;5;220mWINDOWED'*) ok "a windowed station is golden yellow" ;; *) no "a windowed station is golden yellow" "$(slrender | cat -v)" ;; esac
  ...
  case "$(slrender)" in *$'\033[2;38;5;231mCHANNELS'*) ok "the live registry outranks the spawn log" ;; *) no "the live registry outranks the spawn log" "$(slrender | cat -v)" ;; esac
  # A hand-started session has no record and must not be guessed into a window.
  : > "$SLH/spawns.json"
  case "$(slrender)" in *$'\033[2;38;5;220mWINDOWED'*) no "an unrecorded station falls back to tab" "$(slrender | cat -v)" ;; *) ok "an unrecorded station falls back to tab" ;; esac
  ...
  O=$(slrender)
  case "$O" in *$'\033[2;38;5;173mTABBED-A'*) ok "two stations in one window are tabs" ;; *) no "two stations in one window are tabs" "$(printf '%s' "$O" | cat -v)" ;; esac
  case "$O" in *$'\033[2;38;5;173mTABBED-B'*) ok "and so is the other one" ;; *) no "and so is the other one" "$(printf '%s' "$O" | cat -v)" ;; esac
  case "$O" in *$'\033[2;38;5;220mALONE'*) ok "a station alone in its window is golden yellow" ;; *) no "a station alone in its window is golden yellow" "$(printf '%s' "$O" | cat -v)" ;; esac
  ...
  case "$(slrender)" in *$'\033[2;38;5;173mTABBED-A'*) ok "a stale tab count is not over-claimed" ;; *) no "a stale tab count is not over-claimed" "$(slrender | cat -v)" ;; esac
  ...
  case "$(slrender)" in *$'\033[2;38;5;220mTABBED-A'*) ok "and a re-probe promotes it" ;; *) no "and a re-probe promotes it" "$(slrender | cat -v)" ;; esac
  ...
  case "$(slrender)" in *$'\033[2;38;5;220mALONE'*) ok "no probe falls back to the deploy log" ;; *) no "no probe falls back to the deploy log" "$(slrender | cat -v)" ;; esac
  case "$(slrender)" in *$'\033[2;38;5;173mTABBED-A'*) ok "and to tab when neither knows" ;; *) no "and to tab when neither knows" "$(slrender | cat -v)" ;; esac
  ...
  case "$(slas sid-aaa)" in *$'\033[7;38;5;173m MINE '*) ok "your own station is boxed" ;; *) no "your own station is boxed" "$(slas sid-aaa | cat -v)" ;; esac
  case "$(slas sid-aaa)" in *$'\033[7;38;5;173m THEIRS '*) no "nobody else is boxed" "$(slas sid-aaa | cat -v)" ;; *) ok "nobody else is boxed" ;; esac
  # The box moves with the tab: the same fleet, read from the other session.
  case "$(slas sid-bbb)" in *$'\033[7;38;5;173m THEIRS '*) ok "the box follows the reader" ;; *) no "the box follows the reader" "$(slas sid-bbb | cat -v)" ;; esac
